...
@Component({
  selector: 'app-gnd-node',
  imports: [NgDiagramPortComponent, NgDiagramNodeRotateAdornmentComponent],
  templateUrl: './gnd-node.component.html',
  styleUrls: ['./gnd-node.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  hostDirectives: [{ directive: NgDiagramNodeSelectedDirective, inputs: ['node'] }],
  host: { '[class.ng-diagram-port-hoverable-over-node]': 'true' },
})
export class GndNodeComponent implements NgDiagramNodeTemplate<PowerNetData> {
  ...
  netName = computed(() => this.node()?.data?.netName ?? 'GND');
